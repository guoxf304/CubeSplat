...
# 在导入其他模块之前，禁用Tkinter以避免多进程问题
# 这个错误通常发生在子进程结束时，Tkinter试图清理资源但主循环已不存在
os.environ['MPLBACKEND'] = 'Agg'  # matplotlib使用非交互式后端
# 禁用PIL的Tkinter支持（如果存在）
...
class SLAM_GUI:
    def __init__(self, params_gui=None):
        self.step = 0 # 步数
        self.process_finished = False # 是否处理完成
        self.device = "cuda"

        self.frustum_dict = {} # 保存视锥体字典
        self.model_dict = {}

        self.init_widget() # 初始化
        ...
        if params_gui is not None:
            ...
            self.Cube = params_gui.Cube # 关联 cube

        self.gaussian_nums = [] # 高斯点

        self.g_camera = util.Camera(self.window_h, self.window_w) # 创建一个虚拟相机
        self.window_gl = self.init_glfw()
        self.g_renderer = OpenGLRenderer(self.g_camera.w, self.g_camera.h)

        gl.glEnable(gl.GL_TEXTURE_2D) # 开启OpenGL功能
        gl.glEnable(gl.GL_DEPTH_TEST)
        gl.glDepthFunc(gl.GL_LEQUAL)
        self.gaussians_gl = util_gau.GaussianData(0, 0, 0, 0, 0) # 创建用于GPU渲染的高斯点数据

        self.save_path = "." # 保存路径
        self.save_path = pathlib.Path(self.save_path)
        self.save_path.mkdir(parents=True, exist_ok=True)

        threading.Thread(target=self._update_thread).start() # 后台更新
    ...
    def get_current_cam(self):
        '''Open3D GUI 中提取当前相机视角参数, 并构造一个内部使用的 Camera 对象'''
        w2c = cv_gl @ self.widget3d.scene.camera.get_view_matrix() # 获取 world-to-camera 矩阵

        image_gui = torch.zeros( # 模拟图像的大小
            (1, int(self.window.size.height), int(self.widget3d_width))
        )
        vfov_deg = self.widget3d.scene.camera.get_field_of_view() # 获取垂直市场
        hfov_deg = self.vfov_to_hfov(vfov_deg, image_gui.shape[1], image_gui.shape[2]) # 换算得到水平视角
        FoVx = np.deg2rad(hfov_deg) # 转化为弧度
        FoVy = np.deg2rad(vfov_deg)
        fx = fov2focal(FoVx, image_gui.shape[2]) # 计算焦距
        fy = fov2focal(FoVy, image_gui.shape[1])
        cx = image_gui.shape[2] // 2 # 主点的位置
        cy = image_gui.shape[1] // 2
        T = torch.from_numpy(w2c)
        current_cam = Camera.init_from_gui( # 模拟构建了一个Cam对象
            uid=-1,
            T=T,
            FoVx=FoVx,
            FoVy=FoVy,
        ...
        current_cam.update_RT(T[0:3, 0:3], T[0:3, 3]) # 更新位姿
        return current_cam
    ...
    def render_o3d_image(self, results, current_cam):
        if self.depth_chbox.checked:
            ...
            img = cv2.flip(img, 0)  # 让cv2.flip返回新数组，而不是修改原数组
    ...
    def _update_thread(self):
        while True:
            ...
            def update():
                if self.step % 3 == 0: # 每3帧更新一遍场景
                    self.scene_update() # 更新场景
...
def run(params_gui=None):
    ...
    # 抑制Tkinter在多进程环境中的析构警告和错误
    # 这些错误发生在子进程结束时，Tkinter试图清理资源但主循环已不存在
    warnings.filterwarnings('ignore', category=RuntimeWarning, message='.*main thread is not in main loop.*')
    warnings.filterwarnings('ignore', category=RuntimeWarning, message='.*Variable.*__del__.*')
    warnings.filterwarnings('ignore', message='.*tkinter.*', category=RuntimeWarning)
    
    # 设置异常钩子以忽略Tkinter相关的RuntimeError
    ...
    def custom_excepthook(exc_type, exc_value, exc_traceback):
        ...
        if exc_type == RuntimeError:
            ...
            # 检查traceback中是否包含tkinter
            if exc_traceback is not None:
                tb_str = ''.join(traceback.format_tb(exc_traceback))
                if 'tkinter' in tb_str.lower() or 'tkinter/__init__.py' in tb_str:
                    # 检查是否是Variable.__del__中的错误
                    if '__del__' in tb_str and ('Variable' in tb_str or 'Variable' in error_msg):
                        return  # 忽略Variable.__del__中的错误
                    if 'main thread is not in main loop' in error_msg:
                        return  # 忽略主循环相关的错误
