	...
	const bool debug)
{
  ...
  torch::Tensor n_touched = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));    // [P,]
  torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts);                        // [1, H, W]
  torch::Tensor out_opaticy = torch::full({1, H, W}, 0.0, float_opts);                      // [1, H, W]
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
    ...
    const torch::Tensor& colors,            // none 预计算的颜色
	const torch::Tensor& scales,            // [P, 3]
	const torch::Tensor& rotations,         // [P, 4]
	const float scale_modifier,
	const torch::Tensor& cov3D_precomp,     // none 预计算
	const torch::Tensor& viewmatrix,        // [4, 4]
    const torch::Tensor& projmatrix,        // [4, 4]
    const torch::Tensor& projmatrix_raw,    // [4, 4]
    const float tan_fovx,
	const float tan_fovy,
    const torch::Tensor& dL_dout_color,     // [3, W, H]
	const torch::Tensor& dL_dout_depths,    // [1, W, H]
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
