	...
	int * __restrict__ n_touched)
{
	...
	// 当前处理的 tile 对应的高斯起始id和结束id
	uint2 range = ranges[block.group_index().y * horizontal_blocks + block.group_index().x];
	// 将高斯点分批处理，每批最多 BLOCK_SIZE 个
	const int rounds = ((range.y - range.x + BLOCK_SIZE - 1) / BLOCK_SIZE);
	int toDo = range.y - range.x;

	// Allocate storage for batches of collectively fetched data.
	__shared__ int collected_id[BLOCK_SIZE];                // 当前批次的ID
	__shared__ float2 collected_xy[BLOCK_SIZE];             // 当前批次中心点的坐标
	...
	int* n_touched)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		ranges,             // 每个tile受高斯影响的范围
		point_list,         // 按深度和tile排序的高斯索引列表
		W, H,
		means2D,
		colors,
		conic_opacity,
		final_T,            // 渲染过程后每个像素的最终不透明度或透射值
		n_contrib,
		bg_color,
		out_color,
		depth,
