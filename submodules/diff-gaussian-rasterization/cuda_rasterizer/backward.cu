...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs,  float *dL_dtau)
{
	...
	dL_dmeans[idx] += glm::vec3(dL_dmean.x, dL_dmean.y, dL_dmean.z);        // 累加 dL_dmeans

	dL_dtau[6 * idx + 0] += -dL_dmean.x;
	dL_dtau[6 * idx + 1] += -dL_dmean.y;
	dL_dtau[6 * idx + 2] += -dL_dmean.z;                                    // 累加 dL_dtau
	...
	float *dL_dtau)
{
	...
	dL_dmeans[idx] = dL_dmean;                          // 输出 dL_dmeans
	...
	dL_dtau[6 * idx + 5] += dL_dtheta.z;        // 累加 dL_dtau
...
__global__ void preprocessCUDA(
	...
	const float3* dL_dmean2D,   // [P, 2] L 对高斯中心在屏幕位置的梯度，render已计算，输入
	glm::vec3* dL_dmeans,       // [P, 3] L 对高斯点坐标的梯度，输出
	float* dL_dcolor,           // [P, 3] L 对高斯颜色的梯度，render已计算，再次更新
	float *dL_ddepth,           // [P, 1] L 对高斯点深度的梯度，render已计算，输入
	float* dL_dcov3D,           // [P, 6] L 对3D协方差的梯度，输出
	float* dL_dsh,              // [P, M, 3]
	...
	float *dL_dtau)             // [P, 6] L 对位姿的梯度
{
	...
	dL_dmeans[idx] += dL_dmean;                                                     // 类加：dL_dmeans，[P, 3] L 对高斯点坐标的梯度
	...
	for (int i = 0; i < 6; i++) {
		dL_dtau[6 * idx + i] += dL_dt[i];                           // 累加 dL_dtau
	}
	...
	dL_dmeans[idx].z += dL_dpCz * viewmatrix[10];                   // 累加 dL_dmeans

	for (int i = 0; i < 3; i++) {
		float3 c_rho = dp_C_d_rho.cols[i];
		float3 c_theta = dp_C_d_theta.cols[i];
		dL_dtau[6 * idx + i] += dL_dpCz * c_rho.z;
		dL_dtau[6 * idx + i + 3] += dL_dpCz * c_theta.z;                        // 累加 dL_dtau，[P, 6] L 对位姿的梯度
	}
...
renderCUDA(
	const uint2* __restrict__ ranges,           // 范围索引
	const uint32_t* __restrict__ point_list,
	int W, int H,
	const float* __restrict__ bg_color,         // [3,]
	const float2* __restrict__ points_xy_image, // [P, 2] geomState.means2D
	const float4* __restrict__ conic_opacity,   // [conic, opacity] geomState.conic_opacity 二维协方差和不透明度
	const float* __restrict__ colors,           // [P, 3] geomState.rgb 高斯点的颜色
	const float* __restrict__ depths,           // [P, 1] geomState.depths 高斯点的深度
	const float* __restrict__ final_Ts,         // [W, H] imgState.accum_alpha 最终透过率
	const uint32_t* __restrict__ n_contrib,     // [W, H] imgState.n_contrib
	const float* __restrict__ dL_dpixels,       // [3, W, H] L 对像素颜色的梯度
	const float* __restrict__ dL_dpixels_depth, // [1, W, H] L 对深度的梯度
	float3* __restrict__ dL_dmean2D,            // [P, 2] L 对高斯中心在屏幕位置的梯度
	float4* __restrict__ dL_dconic2D,           // [P, 2, 2]
	float* __restrict__ dL_dopacity,            // [P, 1] L 对高斯透明度的梯度
	float* __restrict__ dL_dcolors,             // [P, 3] L 对高斯颜色的梯度
	float* __restrict__ dL_ddepths)             // [P, 1] L 对高斯点深度的梯度
{
	// We rasterize again. Compute necessary block info.
	auto block = cg::this_thread_block();
	auto tid = block.thread_rank();
    
	const uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X; // 水平块数
	// 当前tile左上角像素坐标
	...
	__shared__ int collected_id[BLOCK_SIZE];                // 当前批次的高斯id
	__shared__ float2 collected_xy[BLOCK_SIZE];             // 当前批次值中心点的坐标
	__shared__ float4 collected_conic_opacity[BLOCK_SIZE];  // 协方差矩阵
	__shared__ float collected_colors[C * BLOCK_SIZE];      // 高斯颜色
	__shared__ float collected_depths[BLOCK_SIZE];          // 深度

	__shared__ float2 dL_dmean2D_shared[BLOCK_SIZE];        // L 对高斯点在屏幕坐标的梯度
	__shared__ float3 dL_dcolors_shared[BLOCK_SIZE];        // L 对颜色的梯度
	__shared__ float dL_ddepths_shared[BLOCK_SIZE];         // L 对深度的梯度
	__shared__ float dL_dopacity_shared[BLOCK_SIZE];        // L 对透明度的梯度
	__shared__ float4 dL_dconic2D_shared[BLOCK_SIZE];       // L 对二维协方差的梯度
	...
	// last_contributor 存储着对当前像素的最后一个贡献的高斯在当前range的排序
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{
			const int coll_id = point_list[range.y - progress - 1]; // 每一个高斯id，共有 range.y - range.x 个
			collected_id[tid] = coll_id; // 高斯id存到对应位置
			collected_xy[tid] = points_xy_image[coll_id]; // 高斯中心点在屏幕的位置存到对应位置
			collected_conic_opacity[tid] = conic_opacity[coll_id]; // 二维协方差和不透明度
			#pragma unroll
			for (int i = 0; i < C; i++) {
				collected_colors[i * BLOCK_SIZE + tid] = colors[coll_id * C + i]; // 高斯点的颜色
				
			}
			collected_depths[tid] = depths[coll_id]; // 深度
		}
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			skip |= power > 0.0f; // 如果power >0 就跳过

			const float G = exp(power);
			const float alpha = min(0.99f, con_o.w * G);
			skip |= alpha < 1.0f / 255.0f; // 如果 不透明度小于1/255,也跳过

			if (skip) {
				atomicAdd(&skip_counter, 1); // 每跳过一次，计数加一
			}
			block.sync();
			if (skip_counter == BLOCK_SIZE) { // 当跳过的次数为线程块长度时，跳入下一个批次
				continue;
			}


			T = skip ? T : T / (1.f - alpha); // 反向恢复上一步的T
			const float dchannel_dcolor = alpha * T;

			// Propagate gradients to per-Gaussian colors and keep
			// gradients w.r.t. alpha (blending factor for a Gaussian/pixel
			// pair).
			float dL_dalpha = 0.0f; // 当前像素对当前高斯点透明度的梯度
			...
			// 将block内所有共享内存求和，把0号线程的值变成全block的累积值
...
void BACKWARD::preprocess(
	...
	const glm::vec4* rotations,
	const float scale_modifier,
	const float* cov3Ds,            // geomState.cov3D
	const float* viewmatrix,
	...
	const float tan_fovx, float tan_fovy,
	const glm::vec3* campos,
	const float3* dL_dmean2D,       // [P, 2] L 对高斯中心在屏幕位置的梯度，render已计算
	const float* dL_dconic,         // [P, 2, 2]，render已计算
	glm::vec3* dL_dmean3D,          // [P, 3] L 对高斯点坐标的梯度
	float* dL_dcolor,               // [P, 3] L 对高斯颜色的梯度，render已计算
	float* dL_ddepth,               // [P, 1] L 对高斯点深度的梯度，render已计算
	...
	float* dL_dtau)                 // [P, 6] L 对位姿的梯度
{
	...
	// 计算3D协方差梯度，dL_dcov3D 被计算
	computeCov2DCUDA << <(P + 255) / 256, 256 >> > (
		P,
		means3D,
		radii,
		cov3Ds,     // geomState.cov3D
		focal_x,
		focal_y,
		tan_fovx,
		tan_fovy,
...
void BACKWARD::render(
	...
	const dim3 block,           // block(BLOCK_X, BLOCK_Y, 1)
	const uint2* ranges,        // imgState.ranges 每个tile的开始和结束索引
	const uint32_t* point_list, // binningState.point_list 全局高斯ID
	int W, int H,
	const float* bg_color,      // [3,]
	const float2* means2D,      // [P, 2] geomState.means2D
	const float4* conic_opacity,// [conic, opacity] geomState.conic_opacity 二维协方差和不透明度
	const float* colors,        // [P, 3] geomState.rgb 高斯点的颜色
	const float* depths,        // [P, 1] geomState.depths 高斯点的深度
	const float* final_Ts,      // [W, H] imgState.accum_alpha 最终透过率
	const uint32_t* n_contrib,  // [W, H] imgState.n_contrib
	const float* dL_dpixels,    // [3, W, H] L 对像素颜色的梯度
	const float* dL_dpixels_depth, // [1, W, H] L 对深度的梯度
	float3* dL_dmean2D,         // [P, 2] L 对高斯中心在屏幕位置的梯度
	float4* dL_dconic2D,        // [P, 2, 2]
	float* dL_dopacity,         // [P, 1] L 对高斯透明度的梯度
	float* dL_dcolors,          // [P, 3] L 对高斯颜色的梯度
	float* dL_ddepths)          // [P, 1] L 对高斯点深度的梯度
{
